...
//------EVENT LISTENERS---------//
    ...
    {    var cityQuery=city||cityName.value
        fetch(`https://api.openweathermap.org/data/2.5/weather?q=${cityQuery}&units=metric&appid=ffe54d123cd09567c6544471296b6d05`)
        .then(res=> res.json())
    ...
    {    
            fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${lati}&lon=${longi}&units=metric&appid=ffe54d123cd09567c6544471296b6d05`)
        ...
        {  
           ...
           logTable.innerHTML+=`<tr><td>${name}</td><td>${condition}</td><td>${dataMain.temp_min}</td><td>${dataMain.temp_max}</td><td>${humidity}%</td><td>${(data.wind.speed*3.6).toFixed(1)}</td><td onclick="addBookmark('${name}'); this.firstChild.classList.add('fas'); this.style.pointerEvents='none'"><i class="far fa-bookmark"></i></td></tr>`
            ...
            if(temp>10)
            {
                ...
                conditionImg.src= (data<18 ?' images/coldDay.jpg'  : 'coldNight.jpg')
